...
trap_init(void)
{
    ...
    for (int i = 0; i <= 15; i++){
        SETGATE(idt[IRQ_OFFSET + i], 0, GD_KT, vectors[IRQ_OFFSET + i], DPL_KERN);
    }
	// Per-CPU setup 
	trap_init_percpu();
}
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if (curenv->env_pgfault_upcall){
        ...
        user_mem_assert(curenv, (void*)tf->tf_esp - sizeof(struct UTrapframe), sizeof(struct UTrapframe), PTE_W | PTE_U | PTE_P);
        
        tf->tf_esp = tf->tf_esp - sizeof(struct UTrapframe);
        *(struct UTrapframe*)tf->tf_esp = Utrap;
