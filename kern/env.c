...
region_alloc(struct Env *e, void *va, size_t len)
{
    ...
    for (size_t size = 0; size < len; size += PGSIZE){
        page = page_alloc(false);
        if (page == NULL){
            panic("region_alloc: %e\n", E_NO_MEM);
        }
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	// LAB 3: Your code here.
    lcr3(PADDR(e->env_pgdir)); // idea from pmap.c:231
    ...
    for (; ph < eph; ph++){
        ...
        memcpy((void*)ph->p_pa, (void*)binary + ph->p_offset, ph->p_filesz);
